...
@interface CSConversionVC () <IngredientListViewDelegate>
...
- (id)initWithIngredientGroupIndex:(NSUInteger)ingredientGroupIndex ingredientIndex:(NSUInteger)ingredientIndex
{
  ...
  if (self)
  {
    self.ingredientIndex = [[IngredientsStore shared] flattenedIndexForGroupIndex:ingredientGroupIndex ingredientIndex:ingredientIndex];
    
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(ingredientDeleted:)
                                                 name:IngredientDeleteNotificationName
                                               object:nil];
  }
  return self;
}
...
- (void)markCurrentIngredientAccess
{
  if (self.isViewLoaded && self.view.window != nil) {
    [[IngredientsStore shared] markAccessOfIngredientAtFlattenedIndex:self.ingredientIndex];
  }
}
...
- (NSString *)nameForIngredientAtXOrigin:(CGFloat)xOrigin
{
  ...
  return [[IngredientsStore shared] ingredientNameAtFlattenedIndex:indexOfIngredient];
}

- (void)refreshIngredientNameUI
{
  self.ingredientPickerScrollView.contentSize = CGSizeMake(self.ingredientPickerScrollView.bounds.size.width*[[IngredientsStore shared] flattenedCountOfIngredients],
...
{
  UIViewController* ingredientListVC = [IngredientListViewControllerFactory createWithDelegate: self];
  ingredientListVC.modalPresentationStyle = UIModalPresentationPageSheet;
  [self presentViewController:ingredientListVC animated:YES completion:nil];
}
...
- (void)ingredientListViewSelectedIngredientAtFlattenedIndex:(NSUInteger)flattenedIndex {
  logUserAction(
    @"ingredient_select",
    [[IngredientsStore shared] ingredientAnalyticsDictForIngredientAtFlattenedIndex:flattenedIndex]
  );
  [self selectIngredientAtIndex: flattenedIndex];
...
{
  self.scaleVC.ingredient = [CSIngredient ingredientWithDictionary:
    [[IngredientsStore shared] ingredientDictAtFlattenedIndex:self.ingredientIndex]
  ];
}
